...
class HuggingFaceTransformersPostEncoder(AbsPostEncoder):
    ...
    def __init__(
        ...
        if model_name_or_path == "dummy":
            ...
            if output_size == 0:
                self.odim = input_size
            else:
                self.odim = output_size
        else:
            ...
            self.odim = self.transformer.config.hidden_size

        if input_size != self.odim:
            self.linear_in = torch.nn.Linear(input_size, self.odim)
        else:
