...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        if self.encoder.interctc_use_conditioning:
            if aux_ctc is not None and "utt2lid_token" in aux_ctc.values():
                self.lid_ctc = CTC(
                    odim=140,
                ...
                self.encoder.conditioning_layer = torch.nn.Linear(
                    140, self.encoder.output_size()
                )
            elif ctc.hidden_size != 0:
                self.encoder.conditioning_layer = torch.nn.Linear(
                    ctc.hidden_size, self.encoder.output_size()
                )
            else:
                self.encoder.conditioning_layer = torch.nn.Linear(
        ...
        **kwargs,
        ...
        if self.ctc_weight != 0.0:
            if "src_text" in kwargs:
                kwargs["src_text"] = kwargs["src_text"][
                    :, : kwargs["src_text_lengths"].max()
                ]
                loss_ctc, cer_ctc = self._calc_ctc_loss(
                    encoder_out,
                    encoder_out_lens,
                    kwargs["src_text"],
                    kwargs["src_text_lengths"],
                )
            else:
                loss_ctc, cer_ctc = self._calc_ctc_loss(
                ...
                if self.aux_ctc is not None:
                    ...
                    if idx_key in self.aux_ctc:
                        ...
                        if aux_data_tensor is not None and aux_data_lengths is not None:
                            if aux_data_key == "utt2lid_token":
                                aux_data_tensor = aux_data_tensor.repeat(
                                    1, text.shape[1]
                                )
                                aux_data_tensor[text == self.ignore_id] = self.ignore_id
                                aux_data_lengths = text_lengths
    ...
    def encode(
        ...
        if self.encoder.interctc_use_conditioning:
            encoder_out, encoder_out_lens, _ = self.encoder(
                feats, feats_lengths, ctc=getattr(self, "lid_ctc", self.ctc)
            )
        else:
    ...
    def _calc_att_loss(
        ...
        # 1. Forward decoder
        if "decoder_prefix" in forward_kwargs and "decoder_postfix" in forward_kwargs:
            forward_kwargs["decoder_prefix"][
                forward_kwargs["decoder_prefix"] == -1
            ] = self.ignore_id
            forward_kwargs["decoder_postfix"][
                forward_kwargs["decoder_postfix"] == -1
            ] = self.ignore_id
            ...
            decoder_out, _ = self.decoder(
                ...
                ys_in_pad,
                ys_in_lens,
                prefix=forward_kwargs["decoder_prefix"],
                prefix_lengths=forward_kwargs["decoder_prefix_lengths"],
                postfix=forward_kwargs["decoder_postfix"],
                postfix_lengths=forward_kwargs["decoder_postfix_lengths"],
            )
        else:
            decoder_out, _ = self.decoder(
                encoder_out, encoder_out_lens, ys_in_pad, ys_in_lens
