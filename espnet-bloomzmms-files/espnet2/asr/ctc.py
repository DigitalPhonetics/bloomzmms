...
class CTC(torch.nn.Module):
    ...
    def __init__(
        ...
        if length_adaptor_n_layers > 0:
            ...
            for _ in range(length_adaptor_n_layers):
                length_adaptor_layers.append(
                    torch.nn.Conv1d(encoder_output_size, encoder_output_size, 2, 2)
                )
        ...
        if hidden_size > 0:
            self.hidden_layer = torch.nn.Linear(eprojs, hidden_size)
            eprojs = hidden_size
        else:
            self.hidden_layer = torch.nn.Identity()
    ...
    def _map_hs(self, hs_pad):
        hs_pad = hs_pad.permute(0, 2, 1)
        hs_pad = self.length_adaptor(hs_pad)
        hs_pad = hs_pad.permute(0, 2, 1)

        hs_pad = self.hidden_layer(hs_pad)
    ...
    def forward(self, hs_pad, hlens, ys_pad, ys_lens):
        ...
        """
        hs_pad = self._map_hs(hs_pad)
        hlens = hlens.float().div(self.length_adaptor_ratio).floor().long()
    ...
    def softmax(self, hs_pad):
        ...
        hlen = hs_pad.shape[1]
        hs_pad = self._map_hs(hs_pad)
        result = F.softmax(self.ctc_lo(hs_pad), dim=2)

        if self.hidden_size != 0:
            result = torch.matmul(result, self.ctc_lo.weight)

        if result.shape[1] != hlen:
            result = F.interpolate(result.permute(0, 2, 1), (hlen)).permute(0, 2, 1)
