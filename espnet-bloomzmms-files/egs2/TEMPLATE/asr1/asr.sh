...
train_text_type="text" # or text_int
    ...
    if [ "${num_splits_asr}" -gt 1 ]; then
        ...
        if [ ${#aux_list[@]} != 0 ]; then
            ...
            for aux_dset in "${aux_list[@]}"; do
                 _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${aux_dset},${aux_dset},text_int "
                 if [ -f "${_asr_valid_dir}/${aux_dset}" ]; then
                     _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/${aux_dset},${aux_dset},text_int "
                 fi
            done
        fi
        # shellcheck disable=SC2068
        for i in ${!ref_text_names[@]}; do
            _opts+="--fold_length ${asr_text_fold_length} "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/${ref_text_files[$i]},${ref_text_names[$i]},${train_text_type} "
